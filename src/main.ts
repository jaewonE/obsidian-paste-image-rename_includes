...
interface PluginSettings {
	...
	includeExtensionPattern: string;
...
export default class PasteImageRenamePlugin extends Plugin {
		...
		this.registerEvent(
			this.app.vault.on("create", (file) => {
				...
				if (isPastedImage(file)) {
					debugLog("pasted image created", file.path);
					this.startRenameProcess(file, this.settings.autoRename);
				} else {
					if (this.settings.handleAllAttachments) {
						debugLog("handleAllAttachments for file", file);
						if (this.settings.includeExtensionPattern) {
							if (this.testIncludeExtension(file) == false) {
	...
	testIncludeExtension(file: TFile): boolean {
		const pattern = this.settings.includeExtensionPattern;
		if (!pattern) return false;
		return new RegExp(pattern).test(file.extension);
	}
...
class ImageRenameModal extends Modal {
	...
	onOpen() {
		...
		const getNewPath = (stem: string) =>
			path.join(this.src.parent.path, getNewName(stem));
		...
		const nameSetting = new Setting(contentEl)
			...
			.addText((text) =>
				text.setValue(stem).onChange(async (value) => {
					...
					infoET.children[1].children[1].el.innerText =
						getNewPath(stem);
				})
			);

		const nameInputEl = nameSetting.controlEl
...
class SettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Include extension pattern")
			.setDesc(
				`This option is only useful when "Handle all attachments" is enabled.
			Write a Regex pattern to include certain extensions from being handled. Only the first line will be used. If this option is set, "Exclude extension pattern" will be ignored.`
			)
			.setClass("single-line-textarea")
			.addTextArea((text) =>
				text
					.setPlaceholder("docx?|xlsx?|pptx?|zip|rar")
					.setValue(this.plugin.settings.includeExtensionPattern)
					.onChange(async (value) => {
						this.plugin.settings.includeExtensionPattern = value;
						await this.plugin.saveSettings();
					})
			);
